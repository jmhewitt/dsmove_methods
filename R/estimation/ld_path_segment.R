ld_path_segment = function(path_segment, prev_loc, dims, betaAR) {
  ...
  ll = dsmovetools:::TxModelLd(
    prev_loc = prev_loc - 1,
    cur_loc = path_segment[1, , drop = FALSE] - 1, 
    dst_loc = path_segment[2, , drop = FALSE] - 1,
  ...
  if(path_len > 2) {
    for(i in 2:(path_len-1)) {
      ll = ll + dsmovetools:::TxModelLd(
        prev_loc = path_segment[i - 1, , drop = FALSE] - 1, 
        cur_loc = path_segment[i, , drop = FALSE] - 1, 
        dst_loc = path_segment[i + 1, , drop = FALSE] - 1,
        dims = dims, betaAR = betaAR
      )
    }
  }
