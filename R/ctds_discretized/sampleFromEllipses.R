sampleFromEllipses = function(
  ...
  #
  # resample observations from error ellipse information
  #
  
  flattened_coords = t(apply(flattened_coords, 1, function(r) {
    if(is.finite(r['lon_ind'])) {
      # extract raw error ellipse covariance components
      Sigma_elements = crawl::argosDiag2Cov(
        Major = r['semi_major'], 
        Minor = r['semi_minor'], 
      ...
      sigma_x = exp(Sigma_elements$ln.sd.x)
      sigma_y = exp(Sigma_elements$ln.sd.y)
      sigma_xy = sigma_x * sigma_y * Sigma_elements$error.corr
      Sigma = matrix(
        data = c(sigma_x^2, sigma_xy, sigma_xy, sigma_y^2),
        nrow = 2
      )
      
      # sample distance traveled from observed lon/lat pair (m)
      dxy = mvtnorm::rmvnorm(n = 1, mean = rep(0, 2), sigma = Sigma)
      displacement = sqrt(sum(dxy^2))
      
      # convert inputs to radians
      lon1 = pi * ( r['lon'] / 180 ) 
      lat1 = pi * ( r['lat'] / 180 )
      theta = pi * ( r['orientation'] / 180 ) 
      
      # destination lon/lat given "distance and bearing from start point"
      #  Source: http://www.movable-type.co.uk/scripts/latlong.html
      delta = displacement / 6378388
      lat2 = asin(
        sin(lat1) * cos(delta) + cos(lat1) * sin(delta) * cos(theta)
      )
      lon2 = lon1 + atan2(
        y = sin(theta) * sin(delta) * cos(lat1), 
        x = cos(delta) - sin(lat1 * sin(lat2))
      )
      ...
      # re-map coordinates
      mapped_coords = map_coords(
        lon = lon2, lat = lat2, lon_grid = lons, lat_grid = lats, 
        coord_grid = coords
      )
      
      # munge output
      r['lon'] = lons[mapped_coords$lon_ind]
      r['lat'] = lats[mapped_coords$lat_ind]
      r['semi_major'] = NA
      r['semi_minor'] = NA
      r['orientation'] = NA
      r['sqerror'] = NA
      r['lon_ind'] = mapped_coords$lon_ind
      r['lat_ind'] = mapped_coords$lat_ind
    } 
    r
  }))
